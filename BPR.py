...
@LastEditTime: 2019-05-19 17:35:48
'''
import pandas as pd 
import numpy as np
...
def bpr_mf(user_count, item_count, hidden_dim):
    ...
    # average AUC = mean( accuracy for each user in test set)
    mf_accuracy = tf.reduce_mean(tf.to_float(x>0))
    ...
    return u,i,j,mf_accuracy,bprloss,train_op    
...
with tf.Session() as sess:
    ...
    dim = 100
    u, i, j, mf_accuracy, bprloss, train_op = bpr_mf(user_count, item_count, dim)
    
    tf.global_variables_initializer().run()

    for epoch in range(1, 6):
        total_loss = 0
        for k in range(1, 5000): #输入训练三元组
        ...
        # each batch will return only one user's accuracy
        for t_uij in get_testbatch(user_ratings, user_ratings_test, item_count):

            _accuracy, _test_bprloss = sess.run([mf_accuracy, bprloss],
                                    feed_dict={u:t_uij[:,0], i:t_uij[:,1], j:t_uij[:,2]})
            user_count += 1
            _accuracy_sum += _accuracy
        final_accuracy = _accuracy_sum/user_count
        with open('E:ml/ml-100k/accuracy.txt','a+') as f:
            f.write('dim=%d\taccuracy=%.4f\n' %(dim,final_accuracy))
        print ("test_loss: ", _test_bprloss, "test_accuracy: ", final_accuracy)
        print ("")
